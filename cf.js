...
exports.auth = source => {
    ...
    try {
        child_process.execFileSync("my_cf_cli", ["api", source.api])
...
exports.target = ({ organization, space }) => {
    ...
    try {
        child_process.execFileSync("my_cf_cli", [
            "target",
            "-o",
            organization,
            "-s",
...
exports.appInfo = ({ name, guid }) => {
    try {
        guid =
            guid ||
            child_process
                .execFileSync("my_cf_cli", ["app", "--guid", name])
                .toString()
                .trim()

        const appInfo = child_process
            .execFileSync("my_cf_cli", ["curl", `/v2/apps/${guid}`])
            .toString()
...
exports.appExists = ({ name }) => {
    try {
        child_process.execFileSync("my_cf_cli", ["app", "--guid", name])
        return true
    } catch (e) {
        return false
    }
...
exports.delete = ({ name }) => {
    child_process.execFileSync("my_cf_cli", ["delete", name, "-f"])
    console.log(`CF: Deleted ${name}`)
}

exports.rename = ({ from, to, failOnError = true }) => {
    try {
        child_process.execFileSync("my_cf_cli", ["rename", from, to])
...
exports.start = ({ name }) => {
    ...
    try {
        child_process.execFileSync("my_cf_cli", ["start", name], {
...
exports.log = ({ name }) => {
    try {
        const logs = child_process
            .execFileSync("my_cf_cli", ["logs", "--recent", name])
            .toString()
        return logs
    } catch (e) {
...
exports.stop = ({ name }) => {
    try {
        child_process.execFileSync("my_cf_cli", ["stop", name])
